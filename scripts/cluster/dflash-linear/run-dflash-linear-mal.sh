#!/bin/bash
# 1-GPU offline MAL for the 1-epoch dflash_linear draft.
set -euo pipefail
...
grep -q "def acceptance_along_sequence" "${SPECFORGE_SRC}/specforge/modeling/draft/dflash_linear.py"
test -s "${EVAL_JSONL}"
if [[ -n "${DRAFT_HF}" ]]; then
...
run_docker run --rm --name "${NAME}" \
  ...
  "${RUNTIME_IMAGE}" \
  bash /workspace/SpecForge/scripts/cluster/dflash-linear/inside-dflash-linear-mal.sh
rc=$?
set -e

echo "docker_exit=${rc}"
if [[ ${rc} -ne 0 ]]; then
  echo "FAIL: dflash_linear offline MAL exited ${rc}"
  exit "${rc}"
fi
