#!/bin/bash
# 1-GPU export + serve + 256-prompt holdout eval for dflash_linear.
set -euo pipefail
...
RESULTS_DIR="${RESULTS_DIR:-/shared_nfs/naqin/Linear-Context-DFlash/eval-1epoch}"
HF_HOME="${HF_HOME:-/shared_nfs/naqin/hf-cache}"
HF_HUB_CACHE="${HF_HUB_CACHE:-${HF_HOME}/hub}"
CHECKPOINT="${CHECKPOINT:-/shared_nfs/naqin/Linear-Context-DFlash/train-1epoch/20260918T192223Z/train/qwen3.5-4b-dflash-linear-1epoch-latest}"
EVAL_JSONL="${EVAL_JSONL:-/shared_nfs/naqin/primus-specforge-smoke/sharegpt_eval.holdout.jsonl}"
EVAL_N="${EVAL_N:-256}"
...
echo "EVAL_JSONL=${EVAL_JSONL} EVAL_N=${EVAL_N}"

test -f "${SPECFORGE_SRC}/specforge/modeling/draft/dflash_linear.py"
test -f "${SPECFORGE_SRC}/scripts/eval/dflash_linear_serve.py"
grep -q "def spec_generate" "${SPECFORGE_SRC}/specforge/modeling/draft/dflash_linear.py"
grep -q "last_acceptance_lengths" "${SPECFORGE_SRC}/specforge/modeling/draft/dflash_linear.py"
...
run_docker run --rm --name "${NAME}" \
  ...
  -e EVAL_N="${EVAL_N}" \
  -e BENCH_TIMEOUT="${BENCH_TIMEOUT:-600}" \
  -e MIN_SPEEDUP="${MIN_SPEEDUP:-1.00}" \
  -e MIN_ACCEPT="${MIN_ACCEPT:-1.05}" \
  -e FAIL_ON_GATES="${FAIL_ON_GATES:-0}" \
  -v /shared_nfs:/shared_nfs \
  -v "${SPECFORGE_SRC}:/workspace/SpecForge" \
  "${RUNTIME_IMAGE}" \
  bash /workspace/SpecForge/scripts/cluster/dflash-linear/inside-dflash-linear-eval.sh
rc=$?
set -e

echo "docker_exit=${rc}"
if [[ ${rc} -ne 0 ]]; then
  echo "FAIL: dflash_linear export/serve/eval exited ${rc}"
  exit "${rc}"
fi
