#!/bin/bash
# 1-step dflash_linear smoke on the Qwen3.5-4B shard, inside the Primus overlay image.
set -euo pipefail
...
RESULTS_DIR="${RESULTS_DIR:-/shared_nfs/naqin/Linear-Context-DFlash/smoke}"
...
test -d "${SPECFORGE_SRC}/specforge/algorithms/dflash_linear"
test -s "${SMOKE_DIR}/qwen3.5-4b-dflash-linear-smoke.yaml"
test -s "${SMOKE_DIR}/qwen3.5-4b-dflash-smoke.yaml"
...
NAME="dflash-linear-smoke-${STAMP}"
...
run_docker run --rm --name "${NAME}" \
  ...
  "${RUNTIME_IMAGE}" \
  bash /workspace/SpecForge/scripts/cluster/dflash-linear/inside-dflash-linear-smoke.sh
rc=$?
set -e

echo "docker_exit=${rc}"
if [[ ${rc} -ne 0 ]]; then
  echo "FAIL: dflash_linear smoke exited ${rc}"
  exit "${rc}"
fi

echo "PASS: dflash_linear vs stock DFlash 1-step shard smoke"
