#!/bin/bash
# 1-GPU capture replay for the 1-epoch dflash_linear draft.
set -euo pipefail
...
RESULTS_DIR="${RESULTS_DIR:-/shared_nfs/naqin/Linear-Context-DFlash/capture-replay}"
...
echo "HIDDEN_STATES_PATH=${HIDDEN_STATES_PATH} REPLAY_N=${REPLAY_N}"
echo "SPECFORGE_SRC=${SPECFORGE_SRC}"

test -f "${SPECFORGE_SRC}/specforge/modeling/draft/dflash_linear.py"
test -f "${SPECFORGE_SRC}/scripts/eval/dflash_linear_capture_replay.py"
...
NAME="dflash-linear-replay-${STAMP}"
...
run_docker run --rm --name "${NAME}" \
  ...
  "${RUNTIME_IMAGE}" \
  bash /workspace/SpecForge/scripts/cluster/dflash-linear/inside-dflash-linear-replay.sh
rc=$?
set -e

echo "docker_exit=${rc}"
if [[ ${rc} -ne 0 ]]; then
  echo "FAIL: dflash_linear capture replay exited ${rc}"
  exit "${rc}"
fi
