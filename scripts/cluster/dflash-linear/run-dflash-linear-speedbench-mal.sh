...
echo "ENABLE_THINKING=${ENABLE_THINKING} MT_BENCH_TURNS=${MT_BENCH_TURNS} REPLAY_JSON=${REPLAY_JSON:-none} FEATURE_OFFSET=${FEATURE_OFFSET} FEATURE_SOURCE=${FEATURE_SOURCE}"
...
run_docker run --rm --name "${NAME}" \
  ...
  -e REPLAY_JSON="${REPLAY_JSON}" \
  -e FEATURE_OFFSET="${FEATURE_OFFSET}" \
  -e FEATURE_SOURCE="${FEATURE_SOURCE}" \
