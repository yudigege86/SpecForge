#!/bin/bash
# Install flash-linear-attention[rocm] in the Primus overlay image and run
# GDN/KDA scan/gather tests on one MI355X.
set -euo pipefail
...
RESULTS_DIR="${RESULTS_DIR:-/shared_nfs/naqin/Linear-Context-DFlash/fla-scan}"
...
test -f "${SPECFORGE_SRC}/tests/test_modeling/test_linear_context_scan.py"
...
docker run --rm --name "${NAME}" \
  ...
  -e TRITON_CACHE_DIR="${RESULTS_DIR}/triton-cache" \
  -e RUN_DIR="${RUN_DIR}" \
  -v /shared_nfs:/shared_nfs \
  -v "${SPECFORGE_SRC}:/workspace/SpecForge" \
  "${RUNTIME_IMAGE}" \
  bash /workspace/SpecForge/scripts/cluster/dflash-linear/inside-fla-scan-test.sh
rc=$?
set -e

echo "docker_exit=${rc}"
if [[ ${rc} -ne 0 ]]; then
  echo "FAIL: FLA scan/gather tests exited ${rc}"
  exit "${rc}"
fi

echo "PASS: FLA scan/gather tests"
